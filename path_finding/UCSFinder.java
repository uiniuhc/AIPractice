...
/**
 * this class is for ucs finder
 * @author uiniuhc
 *
 */
public class UCSFinder  implements GraphSearcher {
	...
	UCSFinder(){
		openlist=new TreeSet<int[]>(new Comparator<int[]>(){public int compare(int[] a,int b[]){
          if(a[CURR_COST]==b[CURR_COST]){if(a[CURR_TIME]==b[CURR_TIME]){return a[CURR_NODE]-b[CURR_NODE];}else return a[CURR_TIME]-b[CURR_TIME];}
			return a[CURR_COST]-b[CURR_COST];
			}});
 ...
 */
	...
	public boolean upDateNode(int node, int pre, int cost, int h) {
		...
		if(!nodes.containsKey(node)){
			...
			if(nodes.containsKey(pre)) nnode[CURR_COST]=cost+getCost(pre);
